...
const CreateTaskModal = ({ isOpen, onClose, onTaskCreated, projectId }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.log('Données de la tâche envoyées:', taskData);
                        ...
                        <div className="skills-labels-container">
                            {Array.isArray(skills) && skills.map(skill => {
                                ...
                                const category = Array.isArray(categories) && categories.find(c => 
                                    c.name === skill.category || c.name === skill.category?.name
                                );
                                ...
                                return (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            backgroundColor: isSelected ? (category?.color || '#ccc') : 'transparent',
                                            color: isSelected ? 'white' : (category?.color || '#666')
                                        }}
                                        onClick={() => handleSkillChange(skill.id)}
                                    >
                                        <span className="skill-label-name">{skill.name}</span>
                                        {skill.level && (
                                            <span className="skill-label-level">Niv.{skill.level}</span>
                                        )}
                                        {isSelected && (
